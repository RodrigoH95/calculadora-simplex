class CalculadoraSimplex {
  ...
  start() {
    ...
    this.DOM.btnEliminarLimit.addEventListener("click", (e) => {
      ...
      this.DOM.limitacionesContainer.removeChild(this.DOMlimitacionesContainer.lastChild);
  ...
  completarMatriz() {
    ...
    fila0[0] = -valueX1.value;
    fila0[1] = -valueX2.value;
    this.matrix.push(fila0);
    
    // Se obtiene array con los contenedores de limitaciones
    const limits = Array.from(this.DOM.limitacionesContainer.childNodes);
    for (let i = 0; i < limits.length; i++) {
      // Se extraen los valores en los inputs de cada fila de limitaciones
      const values = Array.from(limits[i].childNodes)
  ...
  mostrarResultado() {
    ...
    this.DOM.crearTabla(this.matrix, ["Base", ...Object.keys(this.incognitas), "T.I"], this.base);
    this.DOM.crearDetalles(this.incognitas, this.interaciones);
  }
  ...
  checkLimitaciones(arrLimits) {
    for (const elem of arrLimits) {
      const valores = Array.from(elem.childNodes).filter(el => el.tagName == "INPUT").map(input => input.value);
